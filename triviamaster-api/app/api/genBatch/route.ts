...
export async function POST(req: NextRequest) {
  try {
    ...
    const { uid, rating = 1200, categories = ["geography"], seen = [], locale = "en", count, perCategory } = body;
    ...
    if (!results.length) return NextResponse.json({ error: "No questions available." }, { status: 500 });

    return NextResponse.json({ items: results });
  } catch (err: any) {
    console.error("genBatch error:", err);
